...
from django.db.models.fields import AutoField, BigIntegerField, IntegerField, PositiveIntegerField, PositiveSmallIntegerField, SmallIntegerField, BooleanField, NullBooleanField, DecimalField, FloatField, CharField, CommaSeparatedIntegerField, EmailField, FilePathField, GenericIPAddressField, IPAddressField, SlugField, URLField, TextField, DateField, DateTimeField, TimeField, NOT_PROVIDED, \
    FieldDoesNotExist
from django.db.models.fields.related import OneToOneField, ManyToManyField, ForeignKey, ManyToOneRel, ManyToManyRel
...
def getSerializer(s, queryset, *args, **kwargs):
    import cyano.serializers

    try:
        return getattr(cyano.serializers, s)(queryset, *args, **kwargs)
...
def get_extra_context(request = [], queryset = None, models = [], template = '', data = {}, species = None):
    ...
    if outformat == 'html':
        ...
        if queryset and len(queryset) > 0 and isinstance(queryset[0], cmodels.Entry):
            ...
            ).order_by("-detail__date").first()
            if entry:
                data['last_updated_date'] = entry.detail.date
            else:
...
def writeExcel(species, queryset, modelArr, is_user_anonymous):
    ...
    wb.properties.creator = 'Generated by CyanoFactory KB on %s at %s' % (
        now.isoformat(), 
        settings.ROOT_URL + reverse('cyano:exportData', kwargs={'species_wid': species.wid}),
        )
...
def format_field_detail_view(obj, field_name, is_user_anonymous, history_id = None):
    ...
    elif isinstance(field, ForeignKey) or isinstance(field, ManyToOneRel):
        if issubclass(field_model, cmodels.Entry):
...
def format_list_html_url(iterable, history_id=None):
    ...
    return format_list_html(
        list(map(lambda x: '<a href="%s" title="%s">%s</a>' % (x.get_absolute_url(history_id), x.get_name_or_wid(), x.get_name_or_wid()), iterable)),
...
def format_sequence_as_html(species, sequence, lineLen=60, seq_offset=0, show_protein_seq=False):
    ...
    line = list(map(lambda x: [x[i:i+3] for i in range(0, len(x), 3)], line))

    if show_protein_seq:
        # Convert to protein seq and split in lists of size lineLen/3
        prot_seq = Seq(str(sequence), IUPAC.unambiguous_dna).translate(table=species.genetic_code)
        ...
        if validate_unique:
            try:
                ...
                errors.append({
                    'type': 'model',
                    'name': model_name,
                    'url': reverse('cyano:listing', kwargs={'species_wid': species.wid, 'model_type': model_name}),
                    'message': format_error_as_html_list(error),
                    })
...
class EmpiricalFormula(dict):
    ...
    def __unicode__(self):
        ...
        txt = ''
        keys = sorted(self.keys(), key=cmp_to_key(compare_molecular_weight))
        for key in keys:
            val = self[key]
            if val == 0:
                continue
