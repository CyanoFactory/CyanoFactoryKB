...
class OptGeneParser(object):
    ...
    def __init__(self, stream=None):
        ...
        self.reactions = []  # type: List[OptGeneParser.Reaction]
    ...
    class Reaction(object):
        def __init__(self, name, reactants=None, reactants_stoic=None, products=None, products_stoic=None, reversible=False, pathways=None, disabled=False):
            if reactants is None:
                reactants = []
            if reactants_stoic is None:
                reactants_stoic = []
            if products is None:
                products = []
            if products_stoic is None:
                products_stoic = []
        ...
        def from_string(string):
            ...
            return OptGeneParser.Reaction(
                name,
                reactants=list(map(lambda x: x[1], reactants)),
                reactants_stoic=list(map(lambda x: x[0], reactants)),
                products=list(map(lambda x: x[1], products)),
                products_stoic=list(map(lambda x: x[0], products)),
                reversible=arrow == "<->",
                disabled=disabled
            )
    ...
    def from_model(model, include_compartment=True, use_name=True):
        ...
        def id_or_name(item):
            if use_name:
                n = item.name if len(item.name) > 0 else item.id
            else:
                n = item.id

            return n

        def metabolite_builder(item):
            if include_compartment:
                n = id_or_name(item) + "[" + item.compartment + "]"
            else:
                n = id_or_name(item)

            met = OptGeneParser.Metabolite.instance(n)
            met.external = item.compartment.lower().startswith("e")

            return met

        for reaction in model.reactions:
            name = id_or_name(reaction)
